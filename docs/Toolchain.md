...
You hand-write an FSM with interfaces in chapter **4.7** and ship a character on it. In **4.13** you meet `Chickensoft.LogicBlocks` and `Godot State Charts`, read their source, and discover hierarchical states, entry/exit guarantees, and serialisable state — three things your version silently got wrong. You then decide. Either answer is defensible; not having an answer is not.
...
2. **Wrap GDScript addons behind a C# interface** the moment you depend on one. One file of ugly `Call()` code, and the rest of your codebase stays clean and testable. Taught in chapter **10.6b**.
...
**The rule that makes this safe:** put every boundary in **one place**. One wrapper file per GDScript addon, exposing a clean C# interface; one GDExtension module with a narrow, documented API. Taught in chapters **10.1b** and **10.6b**.
...
### 6.1 Built-in addons you must enable
...
### 6.2 Built-in *systems* that are libraries in all but name
...
| **⭐ Geometry Nodes** | Scatter addons, procedural modelling addons | B-GN (12.3), 4.x |
| **⭐ Grease Pencil** | Storyboarding software. **Industry-standard previz practice** | **8.2b** |
| **Mantaflow** | Paid smoke/fire sim addons | 6.16 |
| **Cloth / Soft Body / Rigid Body** | Paid sim addons | 6.17 · B38 |
| **QuadriFlow / Voxel Remesh** | Paid auto-retopo (Quad Remesher) | B34 |
| **Video Sequence Editor** | A video editor, for your trailer | 8.24 · 11.19 |
| **Compositor** | Post-processing on renders | 8.24 |
| **OpenColorIO / AgX / Filmic** | Colour management — **the ACES-adjacent industry practice** | **8.24b** |

### 6.3 External free addons worth installing
...
| **Poly Haven addon** | Free | One-click CC0 HDRIs, textures, models into Blender | B15 · 5.11 | — |
...
| **Sverchok** | GPL, free | Parametric node system (Grasshopper-like) | 12.3 🔬 | Geometry Nodes covers most needs now |
| **Camera Shakify** | Free / OSS | Realistic camera-shake presets from real footage | **8.15** | Excellent for cinematics |
...
| **Blender Kitsu** | Free / OSS | Blender Studio's production-tracking addon, pairs with Kitsu | **11.3** | Real studio pipeline |

### 6.4 Free standalone tools that complement Blender
...
| **⭐ FFmpeg** | LGPL/GPL | **Flipbook assembly, trailer encoding, frame extraction.** A genuine professional workhorse | **6.18b** · 11.19 |
...
| **Audacity / Ardour** | Free / OSS | Audio editing; Ardour is a full DAW when Audacity isn't enough | 7.4 · 7.10 |
| **OBS Studio / scrcpy** | Free / OSS | Capture desktop and phone | 6.x · 11.19 |

---

## 6. Godot — the free toolchain

### 7.1 ⭐ Chickensoft — the C# stack
...
|---------|-------------|--------------------|----------|
| **LogicBlocks** | Hierarchical state machines, serialisable, with entry/exit guarantees | 4.7 (your own FSM) | **4.13** |
| **AutoInject** | Dependency injection through the node tree | 1.26 (node lookup patterns) | **10.4b** |
| **GodotNodeInterfaces** | Node interfaces that make scene code **unit-testable** | 10.9 (testing) | **10.9b** |
| **SaveFileBuilder** | Structured save-file composition | 1.33 + 10.7 (your own save system) | **10.7b** |
| **GodotTest** | C# test framework running inside Godot | 10.9 | 10.9 |
...
### 7.2 Core engine features that replace addons
...
### 7.3 Addons worth adopting

⚠️ **GDScript unless marked.** Read §4 before depending on one.

| Addon | Licence | What it does | Hand-build first | Adopt in |
|-------|---------|-------------|-----------------|----------|
| **⭐ Phantom Camera** | MIT | Cinemachine-style camera system: rigs, transitions, look-at, noise | 1.23–1.24 (your own follow cam) | **1.24b** and **8.15b** |
| **⭐ Terrain3D** | MIT, **GDExtension → good C#** | High-performance sculptable terrain with LOD | 5.4 (mesh terrain) | **5.4b** |
| **⭐ Proton Scatter** | MIT | Rule-based scattering: grass, rocks, debris | 5.15 (MultiMesh by hand) | **5.15b** |
| **⭐ Beehave** *or* **LimboAI** | MIT | Behaviour trees for enemy AI. LimboAI is GDExtension (better C# story) | 11.6 (your own state machine AI) | **11.6b** |
| **Godot State Charts** | MIT | Visual statecharts — hierarchical, parallel states | 4.7 | 4.7b (compare with LogicBlocks) |
| **Dialogue Manager** (Nathan Hoad) | MIT, **documented C# support** | Production dialogue system with a script language | 8.9–8.10 (your own) | **8.10b** |
| **Dialogic 2** | MIT | Full visual dialogue editor | 8.9–8.10 | 8.10b (compare) — ⚠️ heavier C# friction |
| **Sky3D** | MIT | Dynamic sky, day/night cycle, clouds | 5.11 | 4.11b 🔬 |
| **Debug Draw 3D** | MIT, GDExtension | Runtime 3D debug drawing — lines, shapes, text | — | **1.11b** — invaluable while learning physics and AI |
| **Panku Console** | MIT | In-game debug console | — | 10.10b 🔬 |
| **Input Helper** | MIT | Device detection, remapping helpers | 1.16 | 11.13 (accessibility) |
| **GdUnit4** | MIT, **C# support** | Test framework, scene-level tests | — | **10.9b** |
| **Zylann HTerrain / Voxel Tools** | MIT | Alternative terrain approaches | — | 5.4b 🔬 |
| **abarichello/godot-ci** | MIT | GitHub Action that builds Godot exports | 11.17 (your own workflow) | **11.17** |
| **Godot Android plugins** (Play Games, Billing) | Varies | Platform services | — | 11.21 🔬 |

### 7.4 NuGet — the C# advantage
...
| **MemoryPack / MessagePack** | Fast binary serialisation when JSON is too slow | 10.7b 🔬 |
| **Serilog** | Structured logging you can actually filter | 9.11b |
| **FluentAssertions** | Readable test assertions | 10.9 |
| **CommunityToolkit.Diagnostics** | Cheap guard clauses | 10.2 |

> ⚠️ **Every NuGet package ships in your APK.** Size and startup cost are real on mobile. Audit before you add — chapter 0.12.

### 7.5 Audio middleware — know it exists
...
| 4.7b | A hand-written FSM | **LogicBlocks**, Godot State Charts | You've felt what a naive FSM gets wrong |
...
| 5.4b | A sculpted mesh terrain | **Terrain3D** | You've hit the limits of a mesh |
| 4.11b 🔬 | A static sky | Sky3D | |
| 5.15b | MultiMesh placed by hand | **Proton Scatter** | You know the draw-call cost you're managing |
| 6.18b | A flipbook assembled by hand | **FFmpeg** | Automating a thing you understand |
| 8.2b | A written outline | **Grease Pencil storyboarding** | Previz, as studios do it |
| 8.10b | Your own dialogue system | **Dialogue Manager** / Dialogic | You know the data model you need |
| 8.15b | Hand-keyed cutscene cameras | **Phantom Camera** + **Camera Shakify** | |
| 8.24b | A graded render | **OpenColorIO / AgX** colour management | |
| 10.4b | Node lookup patterns | **AutoInject** | You've felt the pain of `GetNode` chains |
| 10.7b | A versioned save system | **SaveFileBuilder**, MemoryPack | |
| 10.9b | — | **GdUnit4**, **GodotNodeInterfaces**, FluentAssertions | |
| 9.11b | `GD.Print` everywhere | **Serilog** | |
| 10.3b | A task list | **Kitsu / Blender Kitsu** | Real production tracking |
| 11.6b | A hand-written AI state machine | **Beehave / LimboAI** | You know what the tree is replacing |
| 11.17 | A hand-written CI workflow | **godot-ci** action | |
| 7.2b | Godot's bus system | **FMOD / Wwise** — awareness only | |
...
| **5.2b** | A.N.T. Landscape · Sapling Tree Gen · Cell Fracture |
| **4.2c** 🔬 | Blender GIS |
...
| **11.14b** | Play Games Services · Play Billing |

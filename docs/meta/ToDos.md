...
| T-019 | 🟡 | Source or write the **music** for P01 — one seamless loop | ⬜ | T-002 | Chapter 1.37 walks it. CC0 first; composing your own is a Module 7 option. |
| T-029 | 🟠 | **Sweep published chapters for unlabelled code blocks** ([ADR-037](Decisions.md#adr-037)) — each block needs 📄 NEW FILE / ➕ ADD / 🔁 REPLACE / ✏️ EDIT, and every headerless member block needs its filename in the same sentence | ⬜ | 🤖 | Opportunistic, not one mechanical pass. A bulk regex over every code block in 30 chapters would risk exactly the error class it is meant to prevent. Do a block at a time, at each block close, alongside [T-028](#) |
| T-028 | 🔴 | **Read the learner's committed project code at every block boundary** — [D-018](Doubts.md#d-018) was a spinning marble and a dead exported knob, sitting in `projects/P01_MarbleRunner` unreported. A defect nobody reports is not a defect nobody hit | ⬜ | 🤖 | Standing check. Run at each block close: read every script the block asked for, diff against what the chapters specified, and open a doubt for each divergence. Two of four defects this session were only visible this way |
| T-027 | 🟠 | **Shared snippets for operations repeated across chapters** — environment-variable manipulation, archive extraction, version checks. Each appears in 3–4 chapters and every duplicate is a chance to diverge, which is exactly how [D-016](Doubts.md#d-016) happened: 0.2 had it right, 0.4 had it wrong, written hours apart | ⬜ | 🤖 | Candidate: `docs/reference/snippets/`, referenced rather than copy-pasted |
| T-024 | 🟠 | **Split Module 1** — it is now 63 chapters (Godot foundations + presentation + Android runtime engineering), nearly a fifth of the course. Blocks 1A–1J make it navigable but it should probably be two modules | ⬜ | 👤 decision | Renumbering modules 2–11 touches ~100 cross-references, so this needs a deliberate go-ahead rather than a silent change |
| T-025 | 🟠 | **Decide the capstone scope** — vertical slice as the ship target, or four levels | ⬜ | 👤 | [ADR-019](Decisions.md#adr-019). Explained in full 2026-09-02; awaiting your answer |
...
| T-020 | 🟡 | Decide whether *you* narrate *Ember Hollow*, or it uses TTS, or it has no narrator | ⬜ | — | Not urgent, but it shapes chapter 7.6. Chapters 7.9 and 7.13 give you both paths before you have to choose. |
...
| — | Draft PLAN, TableOfContents, BlenderTrack, projects, Exercises, QuestionBank + answers M0–M3, 5 setup guides, 25 ADRs, meta + internal scaffolding | 2026-09-01 |

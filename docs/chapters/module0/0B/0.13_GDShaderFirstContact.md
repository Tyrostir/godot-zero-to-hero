...
| You need | From |
|---|---|
| P00 with `Spinner.cs` on the cube | [0.11](0.11_CSharpFirstContact.md) |
| The Remote scene tree | [0.6](../0A/0.6_TheGodotEditor.md) Step 5 — you will tune live |
...
> ⚠️ **`Spinner.cs` sets `MaterialOverride` in `_Ready` from [0.11](0.11_CSharpFirstContact.md).** Comment out those three lines, or your C# will replace this material at runtime and you will see nothing.
...
> 💡 **Those hints are the same idea as `[Export(PropertyHint.Range, ...)]` in [0.11](0.11_CSharpFirstContact.md)** — metadata on a declaration, read by the editor to choose a widget. Different language, identical principle.
...
|---|---|---|
| [0.6](../0A/0.6_TheGodotEditor.md) | `GetNode("NoSuchNode")` | Runtime error — at least it complains |
| [0.11](0.11_CSharpFirstContact.md) | Filename ≠ class name | **Silence** |
...
**[0.14 — The language decision table](0.14_LanguageDecisionTable.md).** You have now written all four languages this course uses, or seen where the fourth will arrive. Next you turn your own measurements into a rule you will apply for the next 300 chapters.

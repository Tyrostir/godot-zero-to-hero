...
- ⭐ **Time it.** Edit the value, run, see the change. Do the same in C#. The gap is the build step, and you will quantify it in [0.12](0.12_MeasuredTwoLanguages.md).
...
| You need | From |
|---|---|
| P00 open, with `Main.tscn` and `Spinner.cs` | [0.8](../0A/0.8_P00HelloPhone.md) |
...
The stopwatch matters. Do this properly — you will use the numbers in [0.12](0.12_MeasuredTwoLanguages.md).
...
> 📌 **Do not draw conclusions yet.** Six samples is not an experiment; [0.12](0.12_MeasuredTwoLanguages.md) turns this into one. Just record.
...
Neither is better. They are different points on one trade-off, and [0.14](0.14_LanguageDecisionTable.md) is where you decide which point suits which job.
...
**[0.11 — C# first contact](0.11_CSharpFirstContact.md).** You have written C# already. Next you meet the parts of it that are *not* obvious to someone arriving from C or C++ — properties, attributes, `partial` — and find out what the build step actually buys.

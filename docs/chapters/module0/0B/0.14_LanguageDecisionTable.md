...
- Create `docs/meta/LanguageChoice.md` from the template in Step 2. **Fill it with your numbers from [0.12](0.12_MeasuredTwoLanguages.md)**, not mine.
...
| You need | From |
|---|---|
| Your measurements in `Machines.md` | [0.12](0.12_MeasuredTwoLanguages.md) |
| Having written GDScript and GDShader | [0.10](0.10_GDScriptFirstContact.md), [0.13](0.13_GDShaderFirstContact.md) |
...
| Observation | Chapter |
|---|---|
| C# catches typos at build; GDScript catches them when the line runs | [0.10](0.10_GDScriptFirstContact.md) |
| `rotate_y` and `RotateY` are one function — case conversion is enough | [0.10](0.10_GDScriptFirstContact.md) |
| Godot's conventions (filename, property-not-field) fail **silently** | [0.11](0.11_CSharpFirstContact.md) |
| A shader cannot print, remember, or read nodes | [0.13](0.13_GDShaderFirstContact.md) |
| Parameter and path **strings** are checks the compiler cannot make | [0.6](../0A/0.6_TheGodotEditor.md), [0.11](0.11_CSharpFirstContact.md), [0.13](0.13_GDShaderFirstContact.md) |
...
**Now put numbers on it.** Take your C# iteration median from [0.12](0.12_MeasuredTwoLanguages.md) — call it *n* seconds. A tool script is edited *while you use it*, so a realistic session is dozens of edits. **Dozens of *n*-second waits, versus dozens of instant ones.**

...
- You have written C# since [0.2](../0A/0.2_GodotAndDotNet.md). This chapter is about the parts that are **not obvious coming from C or C++**.
...
| You need | From |
|---|---|
| Both cubes spinning | [0.10](0.10_GDScriptFirstContact.md) |
| The MSBuild / Debugger / Output distinction | [0.6](../0A/0.6_TheGodotEditor.md) |
...
Drag the slider while the scene runs (**Remote** tree, [0.6](../0A/0.6_TheGodotEditor.md) Step 5). Change the colour. Flip the dropdown.
...
That principle recurs: it is why signals are connected in the editor ([0.6](../0A/0.6_TheGodotEditor.md)) rather than by string lookup, and why [9.5](../../../TableOfContents.md) uses typed `Resource` classes rather than loose config.
...
**#3 you already met in [0.2](../0A/0.2_GodotAndDotNet.md)** — but it is worth meeting twice, because it is the single most common "my script does nothing" cause and it produces total silence.
...
**[0.12 — Measured: two languages, one cube](0.12_MeasuredTwoLanguages.md).** You have written the same cube twice and felt the difference. Next you **measure** it — properly, with repetition — and write down numbers you will use for the rest of the course.

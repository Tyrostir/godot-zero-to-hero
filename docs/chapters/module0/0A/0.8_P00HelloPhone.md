...
Plus, in Godot: `Editor → Manage Export Templates` shows templates installed for **your exact version** ([0.2](0.2_GodotAndDotNet.md)), and `Editor → Editor Settings → Export → Android` has all four fields filled ([0.4](0.4_AndroidToolchain.md)).
...
Look at the bottom of the export dialog. If there is a red message about missing export templates, go back to [0.2](0.2_GodotAndDotNet.md) Step 5.
...
Note the total time from pressing deploy to seeing the cube. Write it in [`Journal.md`](../../../meta/Journal.md). **That number is your iteration cost**, and you will pay it thousands of times — which is why chapter [0.5](0.5_ConnectingYourPhone.md) pushed you to set up wireless deployment.
...
This is the same skill as chapter [0.6](0.6_TheGodotEditor.md)'s "which panel spoke", applied to a longer pipeline. **Ask how far it got before asking what went wrong.**
...
**[0.9 — Reading errors: the output panel, the debugger, and `adb logcat`](0.9_ReadingErrors.md).** You have a working pipeline. Next you learn to read it when it stops working — including how to get a **stack trace from code running on the phone**.

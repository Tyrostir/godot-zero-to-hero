...
| You need | From |
|---|---|
| P00 with `Spinner.cs` | [0.11](../0B/0.11_CSharpFirstContact.md) |
| Your language decision table | [0.14](../0B/0.14_LanguageDecisionTable.md) |
| The repo committed and clean | [0.7](../0A/0.7_GitForGameProjects.md) |
...
> 🚨 **`addons/` is committed, not ignored.** A project that does not build after `git clone` is broken. Check your `.gitignore` does not exclude it — [0.7](../0A/0.7_GitForGameProjects.md)'s `git check-ignore -v addons/` should print **nothing**.
...
Use the method from [0.12](../0B/0.12_MeasuredTwoLanguages.md) — this is exactly that skill applied to a dependency.
...
**🔬 P4 — Check the transitive cost.** Does the addon pull in anything else? Godot addons rarely do; NuGet packages ([0.16](0.16_NuGet.md)) very often do, and you are about to meet that.
...
**[0.16 — NuGet](0.16_NuGet.md).** You have judged the ecosystem Godot offers. Next: the far larger one C# gives you, and the fact that every package you add **ships inside your APK**.

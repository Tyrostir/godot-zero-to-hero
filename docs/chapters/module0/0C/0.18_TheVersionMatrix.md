...
| You need | From |
|---|---|
| A failed export from a version mismatch | [0.17](0.17_DevLoopTools.md) Break it |
| `Machines.md` partly filled | [0.1](../0A/0.1_MachinesAndTheirRoles.md)–[0.4](../0A/0.4_AndroidToolchain.md) |

> 📌 **You already know why this matters.** In [0.17](0.17_DevLoopTools.md) you produced a project that opened fine and could not ship, from a single unmatched version. This chapter is the systematic version of that lesson.
...
| `.godot/` cache | ❌ Not committed ([0.7](../0A/0.7_GitForGameProjects.md)) |
...
So you might narrow it to a few candidates, and **you cannot tell `4.7.1` from `4.7.2`** — which is precisely the granularity that matters, because export templates must match *exactly* ([0.17](0.17_DevLoopTools.md)).
...
- Package graph → **`packages.lock.json`** ([0.16](0.16_NuGet.md))
- Hardware and device specs → **`Machines.md`** ([0.1](../0A/0.1_MachinesAndTheirRoles.md))
- Why a dependency is present → **`DecisionsLog.md`** ([0.15](0.15_EvaluatingADependency.md))
...
3. Because **templates must match the editor exactly** — including the release suffix and the .NET variant. [0.17](0.17_DevLoopTools.md)'s Break-it showed a project that opened fine and could not export from precisely that mismatch, and "4.7-ish" cannot distinguish `4.7.1` from `4.7.2`.
...
**[0.19 — Module 0 self-check](0.19_Module0SelfCheck.md).** Every tool is installed, pinned, evaluated and understood. Next you prove it — by rebuilding the whole loop from nothing, without the chapters.

...
> 🚨 **Different file.** Everything above was `Marble.cs`. This step edits **`res://scripts/SpinPlatform.cs`** — the spinner script from [1.4](1.4_YourFirstRealScript.md), attached to `SpinnerGood`. **Open that file before you paste.** ([D-018](../../../meta/Doubts.md#d-018) — the first version of this step showed a fragment with no class header, and it ended up merged into `Marble.cs`.)

Same discipline, whole file:
...
> ⚠️ **If your marble has started spinning, this code is in `Marble.cs`.** Delete the `Motion` group and the `RotateY` line from it. It is not merely cosmetic: **`RotateY` writes the transform directly, and writing a `RigidBody3D`'s transform every physics frame fights the solver** — you are teleporting a body the physics engine is simultaneously trying to simulate. `SpinPlatform` is a `StaticBody3D`, where direct transform writes are fine. [1.13](../../../TableOfContents.md) covers the forces-and-impulses way to move a rigid body, and [1.11](../../../TableOfContents.md) explains why the body type decides which is legal.
...
**P5 — Add a respawn delay, or decide not to.** A short pause before the marble reappears is a real design choice. Implement `RespawnDelay` — count down in `_PhysicsProcess`, respawn at zero — **and only then export it.** Doing it in that order is the whole rule: the code that reads the value exists before the knob that sets it. *(An earlier version of this chapter exported `RespawnDelay` and never read it — precisely the Break-it #1 defect, in the example that warns about it. See [D-018](../../../meta/Doubts.md#d-018).)*
...
| Version | Date | Change |
|---|---|---|
| 1.1 | 2026-09-03 | [D-018](../../../meta/Doubts.md#d-018) — Step 5 showed a headerless fragment that ended up merged into `Marble.cs`; now a whole file, named, with a warning about writing a `RigidBody3D`'s transform. Removed `RespawnDelay`, which was exported and never read — the chapter's own Break-it #1 — and added P5 to implement it deliberately. |

...
4. Click the resource dropdown ▾ → **Save As…**, navigate to `res://resources/`, and type the filename **`Marble.physmat`**.

> ⚠️ **Type the name without `.tres`.** The Save dialog appends the extension itself, so typing `Marble.physmat.tres` gets you **`Marble.physmat.tres.tres`** — which works, and looks like a mistake in every directory listing forever after. ([D-019](../../../meta/Doubts.md#d-019) — this chapter said to type the full name, and produced exactly that.) If you already have the doubled name, rename it in the FileSystem dock rather than in your file manager, so Godot updates the scenes that reference it.
...
**🔁 REPLACE** Step 3's `if` block entirely — do not add this alongside it:

```csharp
        // 🔁 REPLACES the Step 3 block. The line `shared.AlbedoColor = Tint;` must GO.
...
> 🚨 **Check that you deleted `shared.AlbedoColor = Tint;`, not just added the duplicate.** Keeping both *looks* fixed — three colours, exactly as intended — while every marble still writes to the shared material on the way past. The visible symptom is gone and the resource mutation is not, which is the worst of both: **a bug you can no longer see.** ([D-019](../../../meta/Doubts.md#d-019) — this is what happened.)
...
| Version | Date | Change |
|---|---|---|
| 1.1 | 2026-09-07 | [D-019](../../../meta/Doubts.md#d-019) — Step 1 told you to type a filename the Save dialog then extended again (`.tres.tres`); Step 4's Fix A was merged with Step 3's buggy line instead of replacing it. Both found by reading committed code. |
